...
public class TriggerAction extends ActionSupport {
	...
	private void deleteTrigger(Trigger trigger, Job job) throws Exception {
		QuartzClient client = QuartzClientContainer.getQuartzClient(job
				.getQuartzConfigId());
		client.deleteTrigger(job.getSchedulerName(), trigger);
	}

	private void addTrigger(Map<String, Object> triggerMap, Job job)
			throws Exception {
		QuartzClient client = QuartzClientContainer.getQuartzClient(job
				.getQuartzConfigId());
	...
	public void list() throws Exception {
		...
		if (temp == null || temp.size() == 0) {
			...
			JsonUtil.toJson(new Gson().toJson(triggerList));
		}
	}
	...
	public String resume() throws Exception {
		QuartzClient client = QuartzClientContainer.getQuartzClient(job
				.getQuartzConfigId());
		// client.getJmxAdapter().pauseTrigger(client,
		// client.getSchedulerByName(job.getSchedulerName()), trigger);
